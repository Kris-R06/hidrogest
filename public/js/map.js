...
let graficaPPM = null; // NUEVA VARIABLE PARA LA GRÁFICA DE PPM
let bombasGlobal = [];
...
function mostrarGraficas(bomba) {
    ...
    // Destruir gráficas previas para no sobreponerlas
    if (graficaGeneral) graficaGeneral.destroy();
    if (graficaPPM) graficaPPM.destroy();

    const ctxGen = document.getElementById('chartGeneral');
    const ctxPPM = document.getElementById('chartPPM');
    if (!ctxGen || !ctxPPM) return;

    // --- GRÁFICA 1: GENERAL (Todo excepto PPM) ---
    graficaGeneral = new Chart(ctxGen.getContext('2d'), {
        type: 'line',
        data: {
            labels: labels,
            datasets: [
    ...
    // --- GRÁFICA 2: EXCLUSIVA DE PPM ---
    graficaPPM = new Chart(ctxPPM.getContext('2d'), {
        type: 'line',
        data: {
            labels: labels,
            datasets: [
                { 
                    label: 'Sólidos (PPM)', 
                    data: historial.map(r => r.ppm || 0), 
                    borderColor: '#f59e0b', 
                    // Eliminamos el fill y el backgroundColor para que sea solo la línea
                    yAxisID: 'y', 
                    tension: 0.3, 
                    borderWidth: 2 
                }
        ...
        options: {
            ...
            scales: {
                x: { display: true }, 
                y: { type: 'linear', display: true, position: 'right', title: { display: true, text: 'PPM' } }
            }
        }
    });
}

// 5. Función Unificada para Fetch de Datos
async function cargarDatos() {
    try {
        ...
        // Si hay una bomba seleccionada, actualizamos sus gráficas automáticamente
        if (idSeleccionado && bombasGlobal.length > 0) {
            const bomba = bombasGlobal.find(b => b.id === idSeleccionado);
            if (bomba) {
                mostrarGraficas(bomba);
            }
        }
...
function cerrarPanel() {
    ...
    idSeleccionado = null; 
    bombaSeleccionadaId = null;
    cargarDatos(); 
}

// Ejecutar inicial y luego cada 5 segundos
cargarDatos();
